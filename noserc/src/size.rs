...
pub(crate) fn derive_static(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    match &input.data {
        ...
        _ => Err(syn::Error::new(
            input.span(),
            "'SizableStatic' derive does not support this data type!",
        )),
    }
}
...
pub(crate) fn derive_dynamic(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    match &input.data {
        ...
        _ => Err(syn::Error::new(
            input.span(),
            "'SizableDynamic' derive does not support this data type!",
        )),
    }
}
...
pub(crate) fn struct_derive_static(input: DeriveInput, data: &DataStruct) -> crate::DeriveResult {
    ...
    Ok(quote! {
        impl #impl_generics ::noser::traits::Sizable for #name #ty_generics #where_clause {
            type Strategy = ::noser::traits::size::Static;

            fn read_size(arena: &[u8]) -> ::noser::traits::size::ReadReturn<Self> {
                let mut size = 0;
                #(size += <#types as ::noser::traits::Sizable>::static_size();)*
                Ok(size)
            }
        }
    })
    ...
    Ok(quote! {
        impl #impl_generics ::noser::traits::Sizable for #name #ty_generics #where_clause {
            ...
            fn read_size(arena: &[u8]) -> ::noser::traits::size::ReadReturn<Self> {
                ...
                let mut cur_size = 0;
                #(
                    cur_size = <#types as ::noser::traits::Sizable>::read_size(arena).map_err(Into::into)?;
